$(function() {
  ...
  if($(".ansewer").length) {
    ...
    if(yes.length == 0) {
      // Yesの数が、0個の時
      $(".ansewer__title").text("診断できませんでした...");
      $(".ansewer__txt").html("そんなあなたに私が個人的におすすめするボカロPを紹介！<br>ハチ,wowaka,DECO*27,Knaria,ジミーサムP");
    } else if(yes.length == 1) {
			// Yesの数が、1個の時
			$(".ansewer__title").text("Yesの数が１個のあなたにおすすめのボカロPはこちら！");
			$(".ansewer__txt").html("ryo,EasyPoP,鬱P,Orangestar");
    } else if(yes.length == 2) {
			// Yesの数が、2個の時
			$(".ansewer__title").text("Yesの数が２個のあなたにおすすめのボカロPはこちら！");
			$(".ansewer__txt").html("40mP,じん,kemu");
    } else if(yes.length == 3) {
			// Yesの数が、3個の時
			$(".ansewer__title").text("Yesの数が３個のあなたにおすすめのボカロPはこちら！");
			$(".ansewer__txt").html("Neru,ギガ,みきとP");
    } else if(yes.length == 4) {
			// Yesの数が、4個の時
      $(".ansewer__title").text("Yesの数が４個のあなたにおすすめのボカロPはこちら！");
			$(".ansewer__txt").html("n-buna,164,ナユタン星人");
    } else if(yes.length == 5) {
			// Yesの数が、5個の時
			$(".ansewer__title").text("Yesの数が５個のあなたにおすすめのボカロPはこちら！");
			$(".ansewer__txt").html("TOKOTKO(西沢さんP),ryo,kz,ジミーサムP");
    }
  }
